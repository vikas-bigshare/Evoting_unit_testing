    ...
    class Program
    {
        static async Task Main(string[] args)
        {
           ...
           // var ret_romupload = await _obj.Post_Rom_Upload(Company.romupload());
            ...
            /////////////////////////////////////////////////RTA////////////////////////////////
            RTA rTA = new RTA();
            var ret_login1 = await rTA.Post_Login(Company.Default_user());
            var ret_PostUpdateEvent = await rTA.Post_UpdateEvent(RTA.Update_Event());
            //var ret_GetUpdateEvent = await rTA.Get_UpdateEvent();
            // var ret_PostfileUpload = await rTA.Post_FileUpload();
            // var ret_getfileUpload = await rTA.Get_FileUpload(61);
            // var ret_romupload = await rTA.Post_Rom_Upload(Company.romupload());

            //var ret_PostApproved = await rTA.PostApproved_Event(24);
            //var ret_Get_List = rTA _obj.Get_List("rta");
            //var ret_Get_ListPrivateList = rTA _obj.Get_PrivateList("scrutinizer");
            //var ret_Get_EventList = await rTA.Get_EventList("current");
